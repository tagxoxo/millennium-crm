...
export default function WelcomeEmailButton({
  ...
  const canSend = Boolean(policyId || clientId);
  ...
  async function handleSend() {
    ...
    try {
      const res = await fetch("/api/send-welcome-email", {
        ...
        body: JSON.stringify(
          policyId ? { policy_id: policyId } : { client_id: clientId }
        ),
      });
  ...
  return (
    <div className="flex flex-col items-stretch sm:items-end">
      <button
        ...
        title={
          hasEmail
            ? "Preview and send welcome email"
            : "Add client email in Edit Client Info first"
        }
        className="w-full sm:w-auto px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Send Welcome Email
      </button>
