...
const TYPE_COLORS: Record<ContactLog["contact_type"], string> = {
  ...
  non_pay_resolved: "bg-green-500/20 text-green-400 border-green-500/40",
...
export default function ClientContactTimeline({
  contacts,
  policyLabels,
}: ClientContactTimelineProps) {
  if (contacts.length === 0) {
    return (
      <div className="bg-navy-light border border-navy-lighter rounded-xl p-8 text-center">
        <p className="text-gray-400">No communications yet across this client&apos;s policies.</p>
      </div>
    );
  }
            ...
            <div className="bg-navy-light border border-navy-lighter rounded-xl p-4">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                <span className="text-xs text-gray-400">
                  Policy: {policyLabels[contact.policy_id] ?? "—"}
                </span>
              </div>
              {contact.outcome && (
